...
import { FormsModule, NgForm, ReactiveFormsModule, FormGroup, AbstractControl, EmailValidator } from '@angular/forms';
...
describe('AddTodoComponent', () => {
  ...
  describe('The body field', () => {
    ...
    it('should be fine with "blue"', () => {
      bodyControl.setValue('blue');
      expect(bodyControl.valid).toBeTruthy();
    });

    it('should fail on bodys that are too big', () => {
      bodyControl.setValue('D'.repeat(201));
      expect(bodyControl.valid).toBeFalsy();
      expect(bodyControl.hasError('max')).toBeTruthy();
    });

    it('should allow a body to contain non-digits', () => {
      bodyControl.setValue('123x567');
      expect(bodyControl.valid).toBeTruthy();
    });
  });
  ...
  describe('The category field', () => {
    ...
    it('should not allow empty categories', () => {
      ...
      expect(categoryControl.hasError('require')).toBeTruthy();
    });

    it('should be fine with "software development"', () => {
      categoryControl.setValue('software development');
      expect(categoryControl.valid).toBeTruthy();
    });

    it('should fail if exceeding 15 characters', () => {
      categoryControl.setValue('x'.repeat(17));
      expect(categoryControl.valid).toBeFalsy();
      expect(categoryControl.hasError('max')).toBeTruthy();
    });

    it('should allow for digits in the field', () => {
      categoryControl.setValue('x235gbj');
      expect(categoryControl.valid).toBeTruthy();
    });
  });
